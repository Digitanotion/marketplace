...
class messagingManager{
    ...
    function getLastUserMsgsByMsgID($msgID,$pointer){
        ...
        $msgID=$inputValidator->sanitizeItem($msgID, "string");
        //Check if conversation was created
        $sql = "SELECT * FROM mallmsgs WHERE mallMsgID=? and defaultColID=?";
		$stmt = $dbHandler->run($sql, [$msgID,$pointer]);
        if ($stmt->rowCount() > 0) {
            $getMsgs=$stmt->fetch();
			$this->message(1, $getMsgs);
		} 
    ...
    function sendMsgUsrToUsr($usrID,$adID,$recieverUsrID,$UsrMsg,$msgType,$msgID=""){
        ...
        if ($msg_id['status']==1){
            ...
            $stmt1 = $dbHandler->run($sql1, [$msg_id_main, $usrID, $recieverUsrID, $msgTime, $UsrMsg, "1",$msgType]);
            //$stmtData=$stmt->fetch();
            if ($stmt1->rowCount() > 0) {
