...
class FeedbackManager
{
    ...
    function getProductTotalRating($adID){
        ...
        //Check if Ad Exist
            ...
            $sql = "SELECT COUNT(mallUsrRating) AS ratenumber, SUM(mallUsrRating) AS ratetotal FROM mallproductrating WHERE mallAdID=? ORDER BY defaultColID ASC";
            $stmt = $dbHandler->run($sql, [$adID]);
            if ($stmt->rowCount() > 0) {
                $fetch = $stmt->fetch();
                $totalNum = $fetch['ratenumber'] == 0 ? 1 : $fetch['ratenumber'];
                $totalRate = $fetch['ratetotal'];
                $averageRate = $totalRate/$totalNum;
...
function reportAd_mobile($userID,$reason,$reportMessage,$adID){
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = new InputValidator();
        $userID = $inputValidator->sanitizeItem($userID, "string");
        $reason= $inputValidator->sanitizeItem($reason, "string");
        ...
        $reportMessage= $inputValidator->sanitizeItem($reportMessage, "string");
        $reportTime=time();
        $sql = "INSERT INTO malladdispute (mallUsrID, mallAdReportReason, mallAdReportMsg, mallAdReportTime, mallAdID) VALUES (?,?,?,?,?)";
                $stmt = $dbHandler->run($sql, [$userID, $reason, $reportMessage,$reportTime,$adID]);
                if ($stmt->rowCount() > 0) {
                    $this->message(1, "Your report is received, we'll look into it.");
                } else {
                    $this->message(500, "Report not received, an error occurred");
