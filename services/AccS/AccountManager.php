...
class AccountManager
{
    ...
    /* NEW ACCOUNT MOBILE AUTH */
    public function new_user_account_auth($mallUsrfirstName, $mallUsrlastName, $mallUsrEmail, $mallUsrPhone, $mallUsrPassword,$photo,$sso_id)
    {
        //Sanitize inputs
        ...
        if (empty($mallUsrfirstName) || empty($mallUsrlastName) || empty($mallUsrEmail) || empty($mallUsrPhone) || empty($mallUsrPassword)) {
            ...
            $mallUsrPassword = $inputValidator->sanitizeItem($mallUsrPassword, "string");
            $photo = $inputValidator->sanitizeItem($photo, "string");
            $sso_id = $inputValidator->sanitizeItem($sso_id, "string");
            $mallUsrPassword = $securityManagerOb->generateHashBcrypt($mallUsrPassword); //Encrpt and hash password
            //Get DB Info
            $dbHandler = $this->dbHandler;
            //adds new user into the DB. Returns 1 on success, 0 on DB error or 102 on failure if the email and phone number has been used,
            ...
            if ($emailphone_exist['email'] === 1) {
                ...
                $sql = "INSERT INTO mallusrs (  mallUsrID,
                                                       ...
                                                       mallUsrEmail,
                                                       mallUsrPassword,
                                                       mallUsrRegTime,mallUsrPhoto,mallUsr_SSO,mallUsr_SSO_id,mallUsrAccountStatus,mallUsrEmailStatus)  VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
                $stmt = $dbHandler->run($sql, [$mallUsrID, $mallUsrfirstName, $mallUsrlastName, $mallUsrPhone, $mallUsrEmail, $mallUsrPassword, $dateRegistered,$photo,1,$sso_id,1,1]);
                $check_status = $stmt->rowCount();
                //Create Business account for user
                $sqlBiz = "INSERT INTO mallusrbiz (mallUsrID,
                                                       mallBizID,
 ...
 */
    ...
    private function is_emailphone_exist($phone, $email): array
    {
        ...
        $get_user_phone=$stmt_phone->fetch();
        $user_phone=$get_user_phone['mallUsrPhoneNo'];
        ...
        if ($row_count_phone > 0 && $user_phone!="00000000000") {
    ...
    function blockUserAd_mobile(String $usrID, string $adID, $sellerUsrID){
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = $this->inputValidatorOb;
        $securityManagerOb1=new SecurityManager();
        $usrID = $inputValidator->sanitizeItem($usrID, "string");
        $adID = $inputValidator->sanitizeItem($adID, "string");
        $sellerUsrID = $inputValidator->sanitizeItem($sellerUsrID, "string");
        $blockTime=time();
        $blockAgent=$_SERVER['HTTP_USER_AGENT'];
        
        $isSellerBlockedsql = "SELECT * FROM mallblocklists WHERE mallUsrID=? AND mallSellerID=?";
        $isSellerBlockedstmt = $dbHandler->run($isSellerBlockedsql, [$usrID, $sellerUsrID]);
        $isSellerBlocked = $isSellerBlockedstmt->rowCount();

        if ($isSellerBlocked<1){
            //Ad block request to block table
        $sql = "INSERT INTO mallblocklists (mallUsrID,mallSellerID,mallBlockTime,mallBlockAgent) VALUES (?,?,?,?)";
        $stmt = $dbHandler->run($sql, [$usrID, $sellerUsrID, $blockTime,  $blockAgent]);
        $addedToBlockList = $stmt->rowCount();
        if ($addedToBlockList>0){
            $this->message(1, "Seller is blocked");
        }else{
            $this->message(503, "Could not block this seller");
        }
        }
        else{
            $this->message(503, "Seller blocked already");
        }
    ...
    function getAllUsersBlockedByID(String $usrID){
        $sys_message = [];
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = $this->inputValidatorOb;
        //$securityManagerOb1=new SecurityManager();
        $usrID = $inputValidator->sanitizeItem($usrID, "string");
        $isSellerBlockedsql = "SELECT * FROM mallblocklists WHERE mallUsrID=?";
        $isSellerBlockedstmt = $dbHandler->run($isSellerBlockedsql, [$usrID]);
        $isSellerBlocked = $isSellerBlockedstmt->rowCount();

        if ($isSellerBlocked>0){
            if ($isSellerBlocked>0){
                $this->message(1, $isSellerBlockedstmt->fetchAll());
            }else{
                $this->message(404, "Blocked list empty");
            }
        }
        return $this->system_message;
    }
    
     function getAllUsersBlockedCountByID(String $usrID){
        $sys_message = [];
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = $this->inputValidatorOb;
        //$securityManagerOb1=new SecurityManager();
        ...
        if ($isSellerBlocked>0){
                $this->message(1, $isSellerBlocked);
        }else{
                $this->message(404, "0");
            }
     ...
     function unblockUserByID(string $requestUsrID, string $targetUsrID){
        //$sys_message = [];
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = $this->inputValidatorOb;
        //$securityManagerOb1=new SecurityManager();
        $requestUsrID = $inputValidator->sanitizeItem($requestUsrID, "string");
        $targetUsrID = $inputValidator->sanitizeItem($targetUsrID, "string");
        $sql = "DELETE FROM mallblocklists WHERE mallUsrID=? AND mallSellerID=?";

        $stmt = $dbHandler->run($sql, [$requestUsrID, $targetUsrID]);
        $check_status = $stmt->rowCount();
        if ($check_status > 0) {
            $this->message(1, "Seller unblocked successfully");
        } else {
            $this->message(500, "Could not unblock this seller");
        }
        return $this->system_message;
    }

    function isThisUsrBlocked(String $usrID, string $sellerUsrID){
        $dbHandler = new InitDB(DB_OPTIONS[2], DB_OPTIONS[0], DB_OPTIONS[1], DB_OPTIONS[3]);
        $inputValidator = $this->inputValidatorOb;
        //$securityManagerOb1=new SecurityManager();
        $usrID = $inputValidator->sanitizeItem($usrID, "string");
        ...
        if ($isSellerBlocked>0){
                ...
                $this->message(1, "Seller in the list");
            }else{
                $this->message(404, "Seller not in the list");
            }
    ...
    {   
        ...
        $bizSlug = str_replace(' ', '_', strtolower($bizname));

        $sql = "UPDATE mallusrbiz SET mallBizName=?, mallBizSlug=?, mallBizDelivery=?,mallBizAbout=?,mallBizAddress=?,mallBizPostalAddress=? WHERE mallUsrID=?";
        $stmt = $dbHandler->run($sql, [$bizname, $bizSlug, $bizDelivery, $bizAbout, $bizAddress, $bizPostalAddress, $usrID]);
        $check_status = $stmt->rowCount();
     ...
     function updateUsrIDByID($usrID, $docBizID, $dob, $docFile, $docFirstname, $docLastname, $docRequiredBy = null, $docPhone, $usrEmail)
    {
        ...
        $checkExt = []; // checks if any file being uploaded is valid;
        $moved = 0; // checked if all files were moved. 
        $pushName = []; // this contains the names of all uploaded files;

        $fileNames = array_filter($docBizID['name']); 
        $uploader   =   new Uploader();
        $targetDir = $this->paramsOb::MEDIA_STORE . "/idFiles/uploads/";
        $allowTypes = array('jpg','png','jpeg');
        if (!empty($docBizID["name"][0])) {
            foreach($docBizID["name"] as $key=>$val){ 
                // File upload path 
                $name = basename($docBizID["name"][$key]);
                $ext = pathinfo($name, PATHINFO_EXTENSION);

                $fileName = $removeSpaceInFileName.uniqid().".".$ext;

                $targetFilePath = $targetDir . $fileName;

                //loops through all uploading file. pushes 1 to array if any file is not valid
                if(!in_array($ext, $allowTypes)){
                    array_push($checkExt, 1);
                } 
                 
                // Check whether file type is valid 
                $fileType = pathinfo($targetFilePath, PATHINFO_EXTENSION);

                if (count($checkExt) == 0) { // if all files are valid
                    // Upload file to server 
                    if(move_uploaded_file($docBizID["tmp_name"][$key], $targetFilePath)){ 
                        $moved = 1;
                        array_push($pushName, $fileName);                     
                    }else{ 
                        $this->message(500, "Faild to upload. Try again!");
                        $moved = 0; 
                    } 
                    
                } else {
                    $this->message(500,"Invalid file(s)");
                }
            }
            ...
            if ($moved == 1) {
                //start database processes
                $seperateNames = implode(',', $pushName);
                $sql2 = "INSERT INTO mallusridrec (mallUsrID,mallIDDOB,mallIDDocFile,mallIDFirstname,mallIDLastname,mallIDRequiredBy,mallIDPhone,mallIDEmail,mallIDToken) VALUES (?,?,?,?,?,?,?,?,?)";
                $stmt2 = $dbHandler->run($sql2, [$usrID, $dob,  $seperateNames, $docFirstname, $docLastname, $docRequiredBy, $docPhone, $usrEmail, $idToken]);
            ...
            $this->message(500, "no photo selected");
        }
    ...
    public function getKycs()
    {
        ...
        $dbHandler = $this->dbHandler;
        $sql = "SELECT * FROM mallusridrec ORDER BY defaultColID DESC";
        $stmt = $dbHandler->run($sql);
        $check_status = $stmt->rowCount();
        if ($check_status > 0) {
            $this->message(1, $stmt->fetchAll());
