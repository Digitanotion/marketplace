                    ...
                    if ($allIDs['status']==1){
                        foreach ($allIDs['message'] as $id) {
                          ...
                          echo '<tr>
                                ...
                                <td>'.$acctStatMsg.'</td>
                                <td>'.$gender.'</td>
                                <td title="Manually approve and reject emails"> <a onclick="declineEmail('.$mallUsrID.')" class="text-danger mx-1" title="Reject Email"><i class="fas fa-thumbs-down"></i></a>  <a onclick="approveEmail('.$mallUsrID.')" class="text-success
                                 mx-1" title="Approve Email"><i class="fas fa-thumbs-up"></i></a></td>
...
<div id="notify" class="container-fluid" style="display:none; position:fixed; bottom:0; z-index:999999; background-color:white; padding-top:15px; height:max-content; padding-bottom:0;">
    <span style="float:right; color:red; cursor:grab;" onclick="hide('notify')">close</span>
...
<div id="notifyMsg" class="container-fluid" style="display:none; position:fixed; bottom:0; z-index:999999; background-color:white; padding-top:15px; height:max-content; padding-bottom:0;">
    <span style="float:right; color:red; cursor:grab;" onclick="hide('notifyMsg')">close</span>
    <div class="row justify-content-center" style="padding:20px;">
       <form method="POST">
...
<div id="notifyMsgKyc" class="container-fluid" style="display:none; position:fixed; bottom:0; z-index:999999; background-color:white; padding-top:15px; height:max-content; padding-bottom:0;">
    ...
    <div class="row justify-content-center" style="padding:20px;">
       <form method="POST">
              <input type="text" style="padding:20px; width:100%;" placeholder="Reason for disapproval"><br><br>
              <button class="btn btn-success" style="width:100%;">send message</button>
       </form><br>
    </div>
</div>
...
<input id="adminID" hidden value="<?php echo $_SESSION["gaijinmall_user_admin__"]; ?>">
...
function approveEmail(id){
    ...
    var formData = {
      ...
      $.ajax({
      url: "./phpActions/approveEmail.php",
      type: 'post',
      data: formData,
      processData: true,
      dataType: "json",
      ...
      if(data == 1){
        ...
        document.getElementById("resp").innerText = "Email approved successfully";
        // $('.toast').toast('show');
        ...
        document.getElementById("resp").innerText = "Email status already set";
        //  $('.toast').toast('show');
        $('.toast1').fadeIn();
        setTimeout(() => {
          $('.toast1').fadeOut();
            ...
            document.getElementById("resp").innerText = "Error approving email";
            //  $('.toast').toast('show');
            $('.toast1').fadeIn();
            setTimeout(() => {
              $('.toast1').fadeOut();
            ...
            document.getElementById("resp").innerText = "Failed to approve email. Try again";
            // $('.toast').toast('show');
            $('.toast1').fadeIn();
            setTimeout(() => {
              $('.toast1').fadeOut();
...
var formData = {
  ...
  $.ajax({
  url: "./phpActions/declineEmail.php",
  type: 'post',
  data: formData,
  processData: true,
  dataType: "json",
  ...
  if(data == 1){
    ...
    document.getElementById("resp").innerText = "Email rejected";
    // $('.toast').toast('show');
    ...
    setTimeout(() => {
      $('.toast1').fadeOut();
        ...
        document.getElementById("resp").innerText = "Error rejecting email ";
        //  $('.toast').toast('show');
        $('.toast1').fadeIn();
        setTimeout(() => {
          $('.toast1').fadeOut();
        ...
        document.getElementById("resp").innerText = "Failed to reject email. Try again";
        // $('.toast').toast('show');
        $('.toast1').fadeIn();
        setTimeout(() => {
          $('.toast1').fadeOut();
