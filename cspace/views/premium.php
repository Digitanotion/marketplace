...
if(isset($_GET['action'])){
    ...
    if($action == "del"){
        $result = $adManager->setPremiumAdStatus($promoId, 4);
        if($result['status'] == 1){
            $sys_msg['msg_type'] = 1;
            $sys_msg['msg'] = "Premium ad deleted";
        }else{
            $sys_msg['msg_type'] = 500;
            $sys_msg['msg'] = $result['message'];
        }
    }else if($action == "ap"){
        $result = $adManager->setPremiumAdStatus($promoId, 1);
        if($result['status'] == 1){
            $sys_msg['msg_type'] = 1;
            $sys_msg['msg'] = "Premium ad approved";
        }else{
            $sys_msg['msg_type'] = 500;
            $sys_msg['msg'] = $result['message'];
        }
    }else if($action == "dec"){
        $result = $adManager->setPremiumAdStatus($promoId, 2);
        if($result['status'] == 1){
            $sys_msg['msg_type'] = 1;
            $sys_msg['msg'] = "Premium ad declined";
        }else{
            $sys_msg['msg_type'] = 500;
            $sys_msg['msg'] = $result['message'];
        }
...
$adStatistics = $adManager->getPremiumAdStats();
if($adStatistics['status'] == 1){
  ...
  $promoted = $adStatistics['message']['deleted'];
}
          ...
          <div class="card">
            ...
            <div class="card-body">
              <table id="adList" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php 
                  $allAds = $adManager->getPremiumAds();
                    if ($allAds['status']==1){
                        $count = 1;
                        foreach ($allAds['message'] as $adD) {

                          $promotionId = $adD['mallAdPromoID'];
                          //$promoType=$adD['mallAdPromoType'];
                          $getPromoPlanDetails=$adManager->getAdPromotionLists($promotionId)['message'];
                          //echo $promotionId;
                          //var_dump(($getPromoPlanDetails));
                          $promoStat = $adD['mallAdPromoStatus'];
                          $promoDate = date("r",$adD['mallAdPromoStart']);
                          //$promoPlan = "plan ".$adD['mallPromoPlan'];
                          $promoExpDate = date("r", $adD['mallAdPromoEnd']);


                          if($promoStat == 1){
                            $statPromoMsg = '<span class="badge badge-success">Active</span>';
                          }else if($promoStat == 0){
                            $statPromoMsg = '<span class="badge badge-warning">In Review</span>';
                          }else if($promoStat == 2){
                            $statPromoMsg = '<span class="badge badge-danger">Declined</span>';
                          }else if($promoStat == 3){
                            $statPromoMsg = '<span class="badge badge-danger">Expired</span>';
                          }else if($promoStat == 4){
                            $statPromoMsg = '<span class="badge badge-danger">Deleted</span>';
                          }

                          

                          //get the adID
                          $adMain = $adManager->getAdDetails($adD['mallAdID']);
                          $ad = $adMain['message'];
                          ...
                          $adPrice = $adManager::CURRENCY.$adD['mallAdPromoPaid'];//$ad['mallAdPrice'];
                          ...
                          if($adStat != -1){
                            ...
                            <td><a href="premium.php?action=del&id='.$promotionId.'&user='.$mallUsrID.'" class="text-warning mx-1" title="Delete Advert"><i class="fa fa-trash"></i></a> <a href="premium.php?action=ap&id='.$promotionId.'&user='.$mallUsrID.'" class="text-success mx-1" title="Approve Advert"><i class="fas fa-thumbs-up"></i></a> <a href="premium.php?action=dec&id='.$promotionId.'&user='.$mallUsrID.'" class="text-danger mx-1" title="Reject Advert"><i class="fas fa-thumbs-down"></i></a></td>
                            <td>'.$statPromoMsg.'</td>
                            <td>'.$promoDate.'</td>
                            <td><span class="badge badge-dark fs-2">'.strtoupper($getPromoPlanDetails['mallAdPromoName']).'</span></td>
                            <td>'.$promoExpDate.'</td>
