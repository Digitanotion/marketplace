...
class Image implements \ArrayAccess
{
    ...
    protected $imageFullPath; //The compressed image that the web will be using for bigger screens
    protected $thumbFullPath;  //The thumbnail image full path
     ...
     */
    protected $dimensions = array(7000, 7000);
    ...
    public function setThumbWidthHeight($newwidth, $newheight){ 
          $file=$this->fullPath;
          $newfile=$this->getLocation().'/thumbs/'.$this->getName().'.'.$this->getMime();
          ...
          switch(strtolower($this->getMime())){
          case "jpeg":
          ...
          case "png":
            $source = imagecreatefrompng($file);
              ...
              $colorTransparent = imagecolorallocatealpha($thumb, 0, 0, 0, 0x7fff0000);
    ...
    public function setOptWidthHeight($newwidth, $newheight){ 
      $file=$this->fullPath;
      $newfile=$this->getLocation().'/optimized/'.$this->getName().'.'.$this->getMime();
      if(!is_file($file) == true){
      //echo 'Cannot find file, upload it again !';
      //exit();
      }
      ...
      case "gif":
                ...
                imagealphablending($thumb, false);
                imagesavealpha($thumb, true);
                imagecopyresampled($thumb, $source, 0, 0, 0, 0, floor($newwidth), floor($newheight), $width, $height);
                imagegif($thumb, $newfile);
                break;
    ...
    {
      return $this->imageFullPath = $this->getLocation().'/optimized/'.$this->getName().'.'.$this->getMime();
    }
    public function getThumbFullPath()
    {
      return $this->thumbFullPath = $this->getLocation().'/thumbs/'.$this->getName().'.'.$this->getMime();
    }
    /**
