...
if (isset($_POST['usrIDSubmit__btn'])) {
    $editBizResponse = $usrAccManager_ob->updateUsrIDByID($pageUsrID__,$_POST['usrIDType__select'],$_POST['datusrIDDOB__txt'],$_POST['usrIDNo__txt'],$_FILES['usrIDFile__file'],$_POST['usrIDFirstName__txt'],$_POST['usrIDLastName__txt'],"email",$_POST['usrIDEmail__txt']);
    $sys_msg['msg_type'] = $editBizResponse['status'];
    $sys_msg['msg'] = $editBizResponse['message'];
}
if (isset($_POST['edit_email__btn'])){
    $editEmailResponse=$usrAccManager_ob->updateUsrEmailByID($pageUsrID__,$_POST['edit_email__txt']);
                        ...
                        <div class="number_0" id="number0">
                            <form action="" method="POST">
                                <span class="d-flex justify-content-around w-75">
                                    ...
                                    <button class="p-2 text-dark bg-light ms-3 rounded" name="edit_email__btn" <?php echo ($isUsrIDUploaded['status'] == 1) ? 'type="submit"' : 'type="button" onclick="myPhone()"'; ?>><i class="fa fa-pencil m-0 mx-1 fs-title-4"></i></button>
                                </span>
