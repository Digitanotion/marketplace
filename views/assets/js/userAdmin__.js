...
function followVendor(loggedUser,VendorUser){
  followStatus=$("#followVendor___btn").attr('data-follow-status');
      if (loggedUser){
        $.ajax({
          type: "GET",
          url: "../../handlers/allUserOffice.php",
          ...
          .done(function (data) {
            ...
            if (data.status == 1) {
              if (followStatus == "unfollowed") {
                ...
                $("#followVendor___btn").html("Unfollow <i class='fa fa-thumbs-down m-0'></i>");
                $("#followVendor___btn").attr('data-follow-status', 'followed')
                $('#allVendorFollowers').html(data.message);
               }else if(followStatus=="followed"){
                 $("#followVendor___btn").html("Follow <i class='fa fa-thumbs-up m-0'></i>");
                 $("#followVendor___btn").attr('data-follow-status', 'unfollowed')
                 $('#allVendorFollowers').html(data.message);
               }
