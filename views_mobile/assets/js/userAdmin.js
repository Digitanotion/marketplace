...
window.addEventListener("load", () => {
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.register(
      "https://gaijinmall.com/service-worker.js"
    );
  }
});
...
$(".make_offer_btn").on("click", function () {
  if ($(this)[0].hasAttribute("data-bs-toggle")) {
      ...
      window.flutter_inappwebview.callHandler("getPageAlert", "error", "Something Went Wrong", "'So sorry, kindly login tomake an offer. Thanks.'")
  });
...
$(".sendDM").on("click", function () {
  if ($(this)[0].hasAttribute("data-bs-toggle")) {
      ...
      window.addEventListener("flutterInAppWebViewPlatformReady", function(event) {
      window.flutter_inappwebview.callHandler("getPageAlert", "error", "Something Went Wrong", "'So sorry, kindly login to send message to the seller. Thanks.'")
  });
  
    // return showAlert(
    //   "So sorry, kindly login to send message to the seller. Thanks."
    // );
  }
});
$(".reportAd__btn").on("click", function () {
  if ($(this)[0].hasAttribute("data-bs-toggle")) {
      ...
      window.addEventListener("flutterInAppWebViewPlatformReady", function(event) {
      window.flutter_inappwebview.callHandler("getPageAlert", "error", "Something Went Wrong", "'So sorry, kindly login to report this Ad. Thanks.'")
  });
  ...
  $.ajax({
    ...
    beforeSend: function () {
      $("select[name = 'location_city']").append(
        '<option id="loadingOption">Loading Cities..</option>'
      );
    },
    success: function (data) {
      ...
      if (data == 1) {
        if (checkedStatus == 1) {
          window.flutter_inappwebview.callHandler(
            ...
            "success",
            "Chat Enabled",
            "You will receive chats from other users anymore."
          );
          ...
          window.flutter_inappwebview.callHandler(
            ...
            "success",
            "Chat Disabled",
            "You will not receive chats from other users anymore."
          );
        }
      } else if (data == 500) {
        window.flutter_inappwebview.callHandler(
          ...
          "error",
          "",
          "We could not disable chats at the moment"
        );
      } else {
        window.flutter_inappwebview.callHandler(
          "getPageAlert",
  ...
  if (loggedUser) {
    $.ajax({
      ...
      data:
        ...
        loggedUser +
        "&vendor=" +
        VendorUser, // getting filed value in serialize form
    })
      .done(function (data) {
        // if getting done then call.
        console.log(data);
        ...
        if (data.status == 1) {
          ...
          window.addEventListener("flutterInAppWebViewPlatformReady", function(event) {
                window.flutter_inappwebview.callHandler("getPageAlert", "error", "Operation Failed", "Please sign in to follow this user")
              });
            }
            else {
              window.addEventListener("flutterInAppWebViewPlatformReady", function(event) {
                window.flutter_inappwebview.callHandler("getPageAlert", "error", "Operation Failed", "Something went wrong")
              });
          }
      })
      .fail(function (data) {
    ...
    if (e == "confirm") {
      ...
      $.ajax({
        ...
        data:
          ...
          delOtherReason +
          "&delReason=" +
          delReason, // getting filed value in serialize form
      })
        .done(function (data) {
          // if getting done then call.
          console.log(data);
          ...
          if (data == 1) {
            ...
            cuteAlert({
              ...
              message:
                "Sorry, we could not delete your account at moment, try again later.",
              buttonText: "Ok",
            });
          }
...
$(".delAccReason").on("change", function (e) {
  ...
  if (selected == "other") {
    $(".otherReasonContainer").html(
      '<label for="delAccOtherReason" class="">Tell us the reason</label><textarea class="delAccOtherReason d-non w-100 form-control" id="delAccOtherReason" name="delAccOtherReason" placeholder="Tell us the reason" rows="5"></textarea>'
    );
  }
});
